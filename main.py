...
async def signal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        response = f"Сигнал по {pair.upper()}:\n"
        response += f"Направление: {direction.upper()}\n"
        response += f"Вход: {entry}\n"
        response += f"Stop Loss: {sl}\n"
        response += f"Take Profit: {tp}\n"
        response += f"Объём: {mode_settings['amount']} USDT, Плечо: x{mode_settings['leverage']}"
...
async def lesson(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = "Урок на " + today + ":\n\n"
    text += "Сегодняшняя тема: Имбаланс и зоны возврата цены.\n"
    text += "Отметь на графике BTCUSDT последнюю зону неэффективности."
